...
function areUserAndPasswordValid($user, $password) {
    ...
    $result = $stmt->execute() or die ("Invalid query: " . $query . ". Field user introduced is: " . $user);
...
if (isset($_POST['Logout'])) {
    # Delete cookies
    setcookie('user', '', time() - 3600);  // Expira en el pasado para eliminarla
...
?>
    ...
    <section class="auth">
        <div class="message">
            <?= htmlspecialchars($error) ?>  <!-- Evitar XSS al escapar el error -->
        </div>
        <section>
            <div>
                <h2>Login</h2>
